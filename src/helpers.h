...
class VertexBufferObject { //allow you to create buffer on GPU
public:
...
public:
	...
	GLuint program_shader; //pointer to program
	...
	GLint uniform(const std::string &name) const; //gives address of global value

	// Bind a per-vertex array attribute
	GLint bindVertexAttribArray(const std::string &name, VertexBufferObject& VBO) const; //connects a property to vertex attribute
