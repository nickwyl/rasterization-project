...
void VertexBufferObject::update(const Eigen::MatrixXf& M) {
	assert(id != 0); //when you know something is true, should use assert
	glBindBuffer(GL_ARRAY_BUFFER, id); //from CPU to GPU
	glBufferData(GL_ARRAY_BUFFER, sizeof(float)*M.size(), M.data(), GL_DYNAMIC_DRAW); //tells how many bytes we are buffering, then how many you are uploading. then, N.data() gives row pointer to data
	rows = M.rows();
	cols = M.cols();
	check_gl_error(); //check if everything is correct
}
